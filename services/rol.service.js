...
class RolServices {
  ...
  getAll = result => {
    connection.query("SELECT * FROM rol", (err, res) => {
      ...
      console.log("rol: ", res?.rows?.length);
      result(null, res);
    });
  };
  ...
  findById = (id, result) => {
    console.log(id);
    connection.query(`SELECT * FROM rol WHERE ci_paciente LIKE '%${id}%' OR celular_paciente LIKE '%${id}%'`, (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
        return;
  ...
  GetOneById = (id, result) => {
    connection.query(`SELECT * FROM rol WHERE id_rol = ${id}`, (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(err, null);
        return;
  ...
  remove = (id, result) => {
    connection.query("CALL public.crud_rol($1,'DELETE','','')", id, (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
        return;
